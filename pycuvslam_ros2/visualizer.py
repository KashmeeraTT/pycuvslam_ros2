...
class RerunVisualizer:
    ...
    def visualize_frame(
        ...
        # Log trajectory only if it has changed (or periodically - here every frame is fine)
        if len(trajectory) > 0:
        ...
        for i in range(self.num_viz_cameras):
            # Safe access if observations list is shorter than num cameras
            obs = observations_main_cam[i] if i < len(observations_main_cam) else []
            self._log_observations(
                obs, images[i], f"camera_{i}"
            )
